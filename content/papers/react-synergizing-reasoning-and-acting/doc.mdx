...
![Three explorers head for the same flag along three lanes, one wandering off a cliff labelled guess, one stopped at a locked gate labelled stall, and a clay-orange one reaching the flag labelled ground, showing that reasoning alone hallucinates, acting alone gets stuck, and only interleaving the two arrives.](https://0bmg2fhv0km08dlk.public.blob.vercel-storage.com/papers/react-synergizing-reasoning-and-acting/three-paths.webp)
...
![An explorer beside an open empty filing drawer labelled empty while his clay-orange path arrow that was heading straight bends sharply and trails off, labelled derail, showing that when a search returns nothing the reasoning loses its footing and the agent gets stuck.](https://0bmg2fhv0km08dlk.public.blob.vercel-storage.com/papers/react-synergizing-reasoning-and-acting/dead-search.webp)
