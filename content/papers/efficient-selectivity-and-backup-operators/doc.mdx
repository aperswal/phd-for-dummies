...
![Three pixel-art dig sites in a row, two shallow sage holes with a few pebbles and one deeper hole with a glowing clay-orange gem half-buried in dirt, with ink arrows still sending probes into the gem hole, showing that a good move can look bad at first so the search keeps probing it instead of giving up.](/papers/efficient-selectivity-and-backup-operators/selectivity.webp)
...
![A hand-inked dial labelled value sits between a pixel-art scale weighing pebbles on the left, labelled average, and a glowing clay-orange gem on a pedestal on the right, labelled best, with the pointer sliding toward the gem, showing that a node's backed-up value starts as the average of its playouts and slides toward the best one as more playouts arrive.](/papers/efficient-selectivity-and-backup-operators/backup-blend.webp)
