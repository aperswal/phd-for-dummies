...
![Two pixel stepping-stone paths climbing left to right, the upper one marked End only with a single clay-orange star on its last stone, the lower one marked Every step with a clay-orange tick on every stone, showing that an outcome reward scores only the final answer while a dense process reward scores each step.](https://0bmg2fhv0km08dlk.public.blob.vercel-storage.com/papers/process-reinforcement-through-implicit-rewards/sparse-vs-dense.webp)
...
![Two pixel line-chart panels, the left one labelled frozen showing a clay-orange reward curve climbing while a sage truth curve falls away, the right one labelled online showing both curves rising together, showing that a frozen reward gets gamed while a reward that keeps updating on fresh rollouts stays honest.](https://0bmg2fhv0km08dlk.public.blob.vercel-storage.com/papers/process-reinforcement-through-implicit-rewards/reward-hacking.webp)
