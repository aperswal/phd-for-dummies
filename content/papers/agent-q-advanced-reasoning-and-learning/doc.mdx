...
![An explorer stands at the top room of a downward-branching tree of pixel-art rooms, a sage-green lantern guide pointing one ink arrow toward the right branch while the room at the end of that path glows clay-orange and carries a small inked star, showing that the agent searches many possible web pages, listens to a critic that points at the promising branch, and remembers which path reached the goal.](https://0bmg2fhv0km08dlk.public.blob.vercel-storage.com/papers/agent-q-advanced-reasoning-and-learning/hero.webp)
...
![One pixel-art wizard shown twice, on the left sketching three doorways onto the page and on the right writing rank numbers one, two, three beside the same three doors with a quill, the top-ranked door glowing clay-orange, showing that a single model plays both the actor that proposes the next clicks and the critic that grades them.](https://0bmg2fhv0km08dlk.public.blob.vercel-storage.com/papers/agent-q-advanced-reasoning-and-learning/actor-critic.webp)
...
![Two sibling pixel-art doorways at different heights share one room below, an ink brace measures the gap between them, the taller clay-orange door gets a star marked CHOSEN PATH and the shorter sage door gets a cross marked REJECTED PATH, showing that where two sibling moves differ in value by more than a threshold the better one becomes the preferred example and the worse one the rejected example for fine-tuning.](https://0bmg2fhv0km08dlk.public.blob.vercel-storage.com/papers/agent-q-advanced-reasoning-and-learning/preference-pair.webp)
