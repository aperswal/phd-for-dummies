...
![A clay-orange dashed curve labelled surrogate and a sage solid curve labelled true climbing a pixel hill, hugging each other tightly on the left inside a faint shaded band then peeling apart on the right, showing that the local estimate of the improvement matches the real return only while the new policy stays close to the old one.](https://0bmg2fhv0km08dlk.public.blob.vercel-storage.com/papers/trust-region-policy-optimization/surrogate-vs-real.webp)
...
![A pixel signpost standing for the current policy with a dashed clay-orange circle drawn around it labelled K L, and a clay-orange arrow from the signpost that points outward and stops exactly at the circle's edge labelled change, showing that the update is the largest step that still keeps the new policy inside a fixed KL-divergence budget.](https://0bmg2fhv0km08dlk.public.blob.vercel-storage.com/papers/trust-region-policy-optimization/trust-region-step.webp)
