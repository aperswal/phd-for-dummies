...
![A pixel-art adventurer slumped beside a failed task on the left, a glowing clay-orange field journal in the center where a quill writes a lesson learned, and the same adventurer on the right stacking crates to a flag, with ink arrows leading failure to journal to success, showing that the agent improves by writing down what went wrong and reading it before the next try.](https://0bmg2fhv0km08dlk.public.blob.vercel-storage.com/papers/reflexion-language-agents-with-verbal-reinforcement-learning/hero.webp)
...
![A short wooden pixel shelf holding three lesson cards, with a glowing clay-orange new card sliding onto one end and the oldest sage-green card tumbling off the far edge labelled forgotten, showing that memory holds only the last few lessons so an old one falls off when a new one arrives.](https://0bmg2fhv0km08dlk.public.blob.vercel-storage.com/papers/reflexion-language-agents-with-verbal-reinforcement-learning/memory-window.webp)
