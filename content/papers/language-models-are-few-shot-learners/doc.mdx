...
![Three rising ink curves climb over a row of pixel example cards, a tall clay-orange curve for the big model climbing steeply, a short sage curve for the small model staying nearly flat, showing that adding more examples to the prompt helps a large model a lot and a small model almost not at all.](/papers/language-models-are-few-shot-learners/learning-curve.webp)
...
![A pixel scroll with a fixed measuring tape down its edge holds a growing stack of example cards that fills toward a hard line marked full, with one clay-orange card spilling past the edge, showing that the context window is a fixed token budget that caps how many examples can fit in the prompt.](/papers/language-models-are-few-shot-learners/context-budget.webp)
