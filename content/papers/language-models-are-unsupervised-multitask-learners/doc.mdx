...
![A clay-orange pixel reader sits with an open book between a sage scroll of stacked web pages feeding in from the left and three ink tags fanning out to the right labelled answer, know, and any, showing that one reader of the whole internet can answer any kind of question without being taught the task.](https://0bmg2fhv0km08dlk.public.blob.vercel-storage.com/papers/language-models-are-unsupervised-multitask-learners/hero.webp)
...
![A row of blank pixel word tiles runs left to right with the last filled tile glowing clay-orange and a dashed empty slot for the next word, while a sage fan of three candidate tiles points down at the slot with arrows of different thickness and the thickest one winning, showing the model works by guessing the next word over and over.](https://0bmg2fhv0km08dlk.public.blob.vercel-storage.com/papers/language-models-are-unsupervised-multitask-learners/next-token.webp)
