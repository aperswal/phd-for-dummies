...
![A field-journal page with two rows. The top row labelled PPO shows an answer scroll feeding into a sage robot judge holding a gavel. The bottom row labelled GRPO shows a clay-orange group of answer scrolls with a dashed average line through them and, where the judge would sit, only an empty outline crossed out, showing that GRPO drops the separate judge and grades answers against their own group average.](/papers/deepseekmath-pushing-the-limits/hero.webp)
