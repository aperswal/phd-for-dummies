...
![A pixel-art dog mid-stride on a taut clay-orange leash held by a hand, the leash ending at an inked stop-bracket beyond which a faint sage ghost dog runs free, showing that a clipped objective keeps an eager learner on a short leash so it cannot bolt too far in one update even when the reward would pull it all the way to the unleashed goal.](https://0bmg2fhv0km08dlk.public.blob.vercel-storage.com/papers/proximal-policy-optimization/hero.webp)
