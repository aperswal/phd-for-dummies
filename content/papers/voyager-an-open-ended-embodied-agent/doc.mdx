...
![Three small sage pixel icons, a stick, an iron ingot, and a crafting table, flow along ink arrows into one large clay-orange iron pickaxe labelled new skill, showing that a new skill is written by reusing skills already saved in the library.](/papers/voyager-an-open-ended-embodied-agent/skill-library.webp)
...
![Three stacked pixel panels joined by ink arrows, a code scroll with a red cross, an error note reading ERROR BUG, and a fixed code scroll with a green check labelled FIXED APPROVED, showing that the agent reads the error, rewrites the program, and a critic confirms it worked before the skill is saved.](/papers/voyager-an-open-ended-embodied-agent/iterative-prompting.webp)
