...
![A stack of preference cards splits into two inked routes: the long top route runs through a wig-wearing judge holding a scale and loops back before reaching a pixel robot, while the bold bottom route labelled direct goes straight from the cards to the robot, showing that DPO drops the separate reward model and reinforcement learning loop and trains the model in one step.](https://0bmg2fhv0km08dlk.public.blob.vercel-storage.com/papers/direct-preference-optimization/hero.webp)
...
![A pixel robot with a gauge dial sketched inside its chest, the needle leaning toward a clay-orange smiling reply tile and away from a faded one, showing that the language model carries its own reward score as the log-ratio of how much likelier it makes a reply than the reference model.](https://0bmg2fhv0km08dlk.public.blob.vercel-storage.com/papers/direct-preference-optimization/implicit-reward.webp)
...
![Two pixel seesaw scales, the top one already level beside a thin sage nudge arrow and the bottom one tipped the wrong way beside a thick clay-orange shove arrow, showing that DPO leans hardest on the pairs the model currently ranks wrong and barely touches the pairs it already ranks right.](https://0bmg2fhv0km08dlk.public.blob.vercel-storage.com/papers/direct-preference-optimization/sigma-weight.webp)
