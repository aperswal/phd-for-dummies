...
![Two pixel-art notebook pages side by side, the left holding one big crossed-out number labeled blurted answer beside a frowning face, the right holding three small worked lines that flow down an ink arrow into a circled clay-orange final number beside a smiling face, showing that writing out the steps reaches the right answer where blurting one number does not.](https://0bmg2fhv0km08dlk.public.blob.vercel-storage.com/papers/chain-of-thought-prompting/hero.webp)
...
![A vertical chain with sound sage-green links at the top, a cracked clay-orange link in the middle labeled false step, and a shattered link below it, showing that one wrong reasoning step poisons every step that follows.](https://0bmg2fhv0km08dlk.public.blob.vercel-storage.com/papers/chain-of-thought-prompting/broken-chain.webp)
...
![Two sage-green keys crossed out and labeled false paths fail to fit a lock, while one clay-orange key labeled true reasoning turns and opens the same lock, showing that look-alike shortcuts like emitting only the equation or only filler tokens do not give the boost that real worked reasoning does.](https://0bmg2fhv0km08dlk.public.blob.vercel-storage.com/papers/chain-of-thought-prompting/ablations.webp)
